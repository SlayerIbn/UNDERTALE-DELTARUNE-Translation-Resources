...
if (memory == -2 && (!instance_exists(obj_creditbullet)))
{
    ...
    if (st_t >= 320)
    {
        ...
        if (st_t >= 750 && st_y3 > 95 && st_t < 950) // Pacifist fantraslation credits
        {
            st_y2 = -401
            st_f3 += 0.1
            st_y3 -= st_f3
        ...
        if (st_t > 950 && st_y3 < 500)
        {
            st_y2 = -401
            st_y3 += st_f4
            st_f4 += 0.25
        }
        if (st_t > 1100 && st_y3 >= 500)
        {
            if (st_y2 >= -410)
                l_alpha = 0
            depth = -9000
        ...
        draw_set_color(c_yellow)
        scr_drawtext_centered_scaled(320, st_y3, "PLACEHOLDER", 2, 2) // Use # for a second line
    }
}
if (ting == 1)
{
